...
export const useAutoPosition = (ref, x, y, padding = [15, 0, 0, 15]) => {
    ...
    useEffect(() => {
        if (ref.current) {
            const elemBounds = ref.current.firstChild?.getBoundingClientRect();
            const mapBounds = ref.current.closest('.p6o-map-container')?.getBoundingClientRect();
